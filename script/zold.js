...
function listarAlunos(index) {
    ...
    if (index === -2) {
        ...
        for (let i=0; i<(listaAlunos).length;i++) {
            ...
            for (x in listaAlunos[i].email) {str += listaAlunos[x].email + '<br>'}
            str += '</td><td>';
            for (x in listaAlunos[i].tel) {str += listaAlunos[x].email + '<br>'}
            str += '</td></tr>';
            }
        ...
        for (x in listaAlunos[index].email) {str += listaAlunos[x].email + '<br>'}
        str += '</td><td>';
        for (x in listaAlunos[index].tel) {str += listaAlunos[x].tel + '<br>'}
        str += '</td></tr>';
        document.getElementById('tabela-alunos').innerHTML = str;
    }
}
...
function buscarModalidade() {
    ...
    if (cod) {
        for (let i=-1; i<(listaModalidades).length; i++) {
...
function incluirModalidade() {
    ...
    let incDiasDisp = [];
    document.getElementsByClassName('DiasDisp')
...
function listarModalidades(index) {
    ...
    if (index === -2) {
        ...
        document.getElementById('tabela-modalidades').innerHTML =
            '<tr><td>' + listaModalidades[index].CODIGO + '</td><td>'
            + listaModalidades[index].desc + '</td><td>'
            + listaModalidades[index].duracao + '</td><td>';
            + listaModalidades[index].dias_disp + '</td><td>'
            + listaModalidades[index].horarios + '</td><td>'
            + listaModalidades[index].prof + '</td><td>'
            + listaModalidades[index].valor + '</td><td></tr>'
    }
}
...
function buscarPratica() {
    let cpf = document.getElementById('buscador-cpf-pratica').value
    let cod = document.getElementById('buscador-codigo-pratica').value
    if (cpf) {
        for (let i=-1; i<(listaModalidades).length; i++) {
            if (cod === listaModalidades[i].CODIGO) {
                index = i
...
function incluirPratica(iAluno, iModalidade) {
    let incCPF = documento.getElementById('buscador-cpf-pratica')
    let incCODIGO = document.getElementById('buscador-codigo-pratica').value;
    let dias = iModalidade.dias_disp
    let horarios = iModalidade.horarios
    ...
    console.log(listaPraticas);
    fecharPopUp();
    listarModalidades(-2);
}

function abrirPopUp_Praticas(id) {
    let indexPratica = buscarPratica();
    let inHTML;
    let incluirPraticaHTML = '<h1>Adicionar Pratica</h1><hr><p><form action=""><div><br><label for="desc-modalidade">Descrição </label><input type="text" id="desc-modalidade" name="desc-modalidade" placeholder="Descrição da modalidade" required><br><label for="duracao-modalidade">Duração</label><input type="time" id="duracao-modalidade" name="duracao-modalidade" required><br><input type="checkbox" id="dom" name="dom" value="dom"><label for="dom">Domingo</label><input type="checkbox" id="seg" name="seg" value="seg"><label for="seg">Segunda</label><input type="checkbox" id="ter" name="ter" value="ter"><label for="ter">Terça</label><input type="checkbox" id="qua" name="qua" value="qua"><label for="qua">Quarta</label><input type="checkbox" id="qui" name="qui" value="qui"><label for="qui">Quinta</label><input type="checkbox" id="sex" name="sex" value="sex"><label for="sex">Sexta</label><input type="checkbox" id="sab" name="sab" value="sab"><label for="sab">Sábado</label></select><br><label for="horarios-modalidade">Horarios Disponíveis</label><input type="text" id="horarios-modalidade" name="horarios-modalidade" placeholder="Horario separados por vírgula (,)" required><br><label for="prof-modalidade">Professores</label><input type="text" id="prof-modalidade" name="prof-modalidade" placeholder="Professores separados por vírgula (,)" required><br><label for="valor-modalidade">Valor</label><input type="text" id="valor-modalidade" name="valor-modalidade" placeholder="Preço da modalidade" required><br><br><br></div></p><hr><br><div class="btns"><a class="btn" onclick="fecharPopUp();">fechar</a><a id="enviar-modalidade" class="btn" onclick="incluirModalidade();">finalizar</a></div></form>';
    let alterarPraticaHTML = '<h1>Alterar Modalidade</h1><hr><p><form action=""><div><br><label for="desc-modalidade">Descrição </label><input type="text" id="desc-modalidade" name="desc-modalidade" placeholder="Descrição da modalidade" required><br><label for="duracao-modalidade">Duração</label><input type="time" id="duracao-modalidade" name="duracao-modalidade" required><br><input type="checkbox" id="dom" name="dom" value="dom"><label for="dom">Domingo</label><input type="checkbox" id="seg" name="seg" value="seg"><label for="seg">Segunda</label><input type="checkbox" id="ter" name="ter" value="ter"><label for="ter">Terça</label><input type="checkbox" id="qua" name="qua" value="qua"><label for="qua">Quarta</label><input type="checkbox" id="qui" name="qui" value="qui"><label for="qui">Quinta</label><input type="checkbox" id="sex" name="sex" value="sex"><label for="sex">Sexta</label><input type="checkbox" id="sab" name="sab" value="sab"><label for="sab">Sábado</label></select><br><label for="horarios-modalidade">Horarios Disponíveis</label><input type="text" id="horarios-modalidade" name="horarios-modalidade" placeholder="Horario separados por vírgula (,)" required><br><label for="prof-modalidade">Professores</label><input type="text" id="prof-modalidade" name="prof-modalidade" placeholder="Professores separados por vírgula (,)" required><br><label for="valor-modalidade">Valor</label><input type="text" id="valor-modalidade" name="valor-modalidade" placeholder="Preço da modalidade" required><br><br><br></div></p><hr><br><div class="btns"><a class="btn" onclick="fecharPopUp();">fechar</a><a id="enviar-modalidade" class="btn" onclick="alterarModalidade('+indexModalidade+');">finalizar</a></div></form>';
    let excluirPraticaHTML = '<h1>Excluir modalidade</h1><hr><br><p>Você tem certeza de que quer excluir os dados desse modalidade? Todas os registros de pratica e informações serão deletados permanentemente do sistema!</p><br><hr><br><div class="btns"><a class="btn" onclick="excluirModalidade();fecharPopUp();">Excluir Modalidade</a><a class="btn" onclick="fecharPopUp();">fechar</a>';
    let praticaVaziaHTML = '<h1>Digite o CODIGO...</h1><hr><br><p>Você não digitou o CODIGO do modalidade que está procurando!</p><br><hr><br><div class="btns"><a class="btn" onclick="fecharPopUp();">fechar</a>';
    let pratica404HTML = '<h1>Prática não encontrada...</h1><hr><br><p>Este CODIGO ou CPF não é de nenhuma das práticas cadastradas...</p><br><hr><br><div class="btns"><a class="btn" onclick="fecharPopUp();">fechar</a>';
    let praticaJaCadastradaHTML = '<h1>Prática já registrada...</h1><hr><br><p>Este CODIGO e CPF pertencem a uma das práticas já cadastradas...</p><br><hr><br><div class="btns"><a class="btn" onclick="fecharPopUp();">fechar</a>';

    if (indexPratica === -2) {
        if (id === "listar-pratica") {
            listarPraticas(indexPratica)
            return indexPratica
        } else {
            inHTML = modalidadeVaziaHTML
        }
    } else if (indexPratica === -1) {
        if (id === 'incluir-pratica') {
            inHTML = incluirModalidadeHTML
        } else if (id === 'alterar-pratica' || id === 'excluir-pratica' || id ==='listar-pratica') {
            inHTML = modalidade404HTML
        }
    } else {
        if (id === 'incluir-pratica') {
            inHTML = modalidadeJaCadastradaHTML
        } else if (id === 'excluir-pratica') {
            inHTML = excluirModalidadeHTML
        } else if (id === 'listar-pratica') {
            listarModalidades(indexPratica);
            return indexPratica
        } else if (id === 'alterar-pratica') {
            inHTML = alterarModalidadeHTML;
        }
    }
    document.getElementById('modal-conteudo').innerHTML = inHTML
...
function listarPraticas(index) {
    console.log('listar pratica' + index)
    if (index === -2) {
        document.getElementById('tabela-modalidades').innerHTML = ''
        for (let i=0; i<(listaModalidades).length;i++) {
            document.getElementById('tabela-modalidades').innerHTML +=
            ...
            + listaModalidades[i].horarios + '</td><td>'
            + listaModalidades[i].prof + '</td><td>'
            + listaModalidades[i].valor + '</td></tr>'
        }
    } else {
        ...
        document.getElementById('tabela-modalidades').innerHTML =
            ...
            + listaModalidades[index].horarios + '</td><td>'
            + listaModalidades[index].prof + '</td><td>'
            + listaModalidades[index].valor + '</td></tr>'
    }
}
